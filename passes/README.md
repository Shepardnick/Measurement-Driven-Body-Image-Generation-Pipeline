...
| 6   | [end-to-end-fitting](pass-6-end-to-end-fitting/v2.md) | v2 + [report](pass-6-end-to-end-fitting/report.md) | **executed** | All four phases done. Pyrender PBR renderer (smooth-shaded skin, proper lighting). Autograd-optimizer over MHR body-only components (first 20 of 45) — fixes the head-distortion-when-fitting-body problem. Plugin body source (`measurements` or `obj`). `scripts/build_body.py configs/<body>.json` single entry point. SMPL-X-era code deleted (a2b, anthropometry, mesh_correction, smplx_body, build_corrected_body, smoke_test_adapters). End-to-end residuals on subject_a: stature/bust/waist/hip all within 0.5 cm. **First render that looks like a real person, not a mannequin caricature.** |
