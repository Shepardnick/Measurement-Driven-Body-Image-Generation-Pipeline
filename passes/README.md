...
| 9   | [skin-texture-and-lighting](pass-9-skin-texture-and-lighting/v1.md) | v1 + [report](pass-9-skin-texture-and-lighting/report.md) | executed | UV extraction from `char.blend` via headless Blender; albedo texture baked to per-vertex colors as workaround for a PyOpenGL+EGL+Python-3.12 `glGenTextures` ctypes bug. Bright-global lighting rig (ambient 0.55, 4 balanced lights, no rim). Renders show realistic skin tone, nipple/areola/lip/eye coloration, even lighting that reveals defects (per user request: no harsh shadows hiding problems). Body still T-pose/bald/nude; hair/clothing/facial-identity/SSS explicitly out of scope. |
