...
| 6   | [end-to-end-fitting](pass-6-end-to-end-fitting/v1.md) | v1 | **draft — awaiting feedback** | Single-command photo-to-body pipeline. Sam3D as primary fitting (Meta's blessed image→MHR tool), autograd-optimizer fallback when no GPU. Per-body JSON config replaces hardcoded paths. Explicit identity_adjustment.deltas layer surfaces the Meta-known bust/butt under-fit as a config knob, not a code patch. Cleans up dead SMPL-X-era code. |
